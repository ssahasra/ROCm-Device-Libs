 ...
 *  terminator.
 *
 *  A null pointer is transmitted as a single zero, which makes it
 *  indistinguishable from an empty string to the host-side receiver.
 *
 ...
 */
ulong
__ockl_printf_append_string(ulong msg_desc, const char *data, uint is_last)
{
    long2 retval = {0, 0};
    ...
    // Short-circuit a null pointer; send a single zero instead.
    if (!data) {
    ...
    // Compute length including the null terminator.
    const char *ptr = data;
    while (*ptr++);
    // Length includes the null terminator due to the post-increment.
    ulong len = (ulong)(ptr - data);
