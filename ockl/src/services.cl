...
{
    return __ockl_hostcall_preview(SERVICE_FUNCTION_CALL,
...
#define PACK_ULONG(ARG)                                                        \
    ...
    long2 result = __ockl_hostcall_preview(service_id, msg_desc, arg1, arg2,
 ...
 */
static long2
message_append_bytes(uint service_id, ulong msg_desc,
                     const uchar *data, ulong len)
{
    ...
    do {
        ...
        retval = append_bytes(service_id, desc, data, plen);
        desc = (ulong)retval.x;
        len -= plen;
        data += plen;
    } while (len != 0);
 ...
 */
static long2
message_append_args(uint service_id, ulong msg_desc,
                    uint num_args, ulong arg0, ulong arg1,
                    ulong arg2, ulong arg3, ulong arg4,
                    ulong arg5, ulong arg6)
{
    msg_desc = msg_set_len(msg_desc, num_args);

    long2 result = __ockl_hostcall_preview(service_id, msg_desc, arg0, arg1,
...
ulong __ockl_printf_begin(ulong version, ulong num_printf_args, ulong fmt_len)
{
    const uint num_args = 3;
    const ulong msg_desc = msg_set_begin_flag(0);
    long2 retval = message_append_args(SERVICE_PRINTF, msg_desc, num_args,
                                       version, num_printf_args, fmt_len,
                                       0, 0, 0, 0);
    return retval.x;
}

ulong
__ockl_printf_append_bytes(ulong msg_desc, const char *data, ulong len, uint is_last)
{
    if (is_last) {
...
ulong
__ockl_printf_append_arg(ulong msg_desc, ulong arg0, ulong arg1, uint is_last)
{
    if (is_last) {
        msg_desc = msg_set_end_flag(msg_desc);
    }
